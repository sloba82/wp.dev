...
abstract class Routines
{
    ...
    public static function handleUnpaidPayments()
    {
        ...
        if ( $timeout ) {
            ...
            $rows = Payment::query( 'p' )
                ->select( 'p.id, p.details' )
                ->where( 'p.type', Payment::TYPE_CLOUD_STRIPE )
                ->where( 'p.status', Payment::STATUS_PENDING )
        ...
        $payments = Proxy\Shared::getOutdatedUnpaidPayments( $payments );
        if ( $payments ) {
            $ca_ids = array();
