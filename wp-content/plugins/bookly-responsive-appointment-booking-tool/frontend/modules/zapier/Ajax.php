...
class Ajax extends Lib\Base\Ajax
{
    ...
    private static function getAppointments()
    {
        ...
        return array_map( function ( $appointment ) use ( $all_extras ) {
            ...
            // Custom fields
            foreach ( (array) Lib\Proxy\CustomFields::getForCustomerAppointment( new Lib\Entities\CustomerAppointment( array( 'custom_fields' => $appointment['custom_fields'] ) ) ) as $cf ) {
    ...
    private static function getAppointmentsQuery( $with_updated )
    {
        ...
        if ( $with_updated ) {
            $query
                ->select( 'CONCAT(ca.id,\'-\',UNIX_TIMESTAMP(GREATEST(ca.created_at,a.updated_at,COALESCE(p.updated_at,0)))) AS id' )
                ->whereRaw(
                'ca.created_at >= \'%s\' 
                OR a.updated_at >= \'%s\'
                OR p.updated_at >= \'%s\'
        ...
        $query->addSelect( 'a.internal_note, a.start_date, DATE_ADD(a.end_date, INTERVAL IF(ca.extras_consider_duration, a.extras_duration, 0) SECOND) AS end_date,
            ...
            GREATEST(ca.created_at,a.updated_at,COALESCE(p.updated_at,0)) AS updated_at' )
